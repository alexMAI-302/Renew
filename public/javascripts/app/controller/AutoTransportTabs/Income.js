Ext.define('app.controller.AutoTransportTabs.Income', {
	...
	stores: [
		...
		'AutoTransport.Income.Ggroup',
		'AutoTransport.Income.Goods',
			...
			'#addIncGoods':{
				click: function(){
					var sm=Ext.getCmp('incomeTable').getSelectionModel(),
						r = Ext.ModelManager.create({at_income: sm.getSelection()[0].get('id')}, 'app.model.AutoTransport.IncGoodsModel');
	...
	initStores: function(){
		...
		controller.ggroupStore=controller.getAutoTransportIncomeGgroupStore();
		controller.goodsStore=controller.getAutoTransportIncomeGoodsStore();
	...
	initTables: function(){
		...
		goodsColumn.field.addListener(
			...
			function(combo, selected, eOpts){
				...
				groupColumn.field.select((selected[0]!=null)?selected[0].get('at_ggroup'):null);
				return true;
			}
		);
