...
class RenewUsersController < ApplicationSimpleErrorController
	...
	def renew_user
		begin
			case request.method.to_s
				...
				when "post"
					renew_user = RenewUser.new("id" => params[:id], "name"=>params[:name])
					renew_user.update_attribute("id", params[:id])
					renew_user.save
	...
	def renew_group
		begin
			case request.method.to_s
				...
				when "post"
					renew_user = RenewUserGroup.new("id" => params[:id], "name"=>params[:name])
					renew_user.update_attribute("id", params[:id])
					renew_user.save
					render :text => {"success" => true, "id" => renew_user.id}.to_json
				when "put"
	...
	def renew_users_groups
		begin
			case request.method.to_s
				when "get" 
					if params[:renew_user_id]
						render :text => RenewUsersGroup.find(:all, :conditions => [ "renew_user_id = ?", params[:renew_user_id]]).to_json
					else 
						if  params[:renew_user_group_id]
							render :text => RenewUsersGroup.find(:all, :conditions => [ "renew_user_group_id = ?", params[:renew_user_group_id]]).to_json
						else
							puts "Error!!! #{t}"
							render :text => {"success" => false, "msg" => "Нет ожидаемого параметра (id группы или id пользователя)"}.to_json
						end
					end
				when "post"
	...
	def renew_groups_urls
		begin
			case request.method.to_s
				when "get" 
					render :text => RenewUsersUrl.find(:all, :conditions => [ "renew_user_group_id = ?", params[:renew_user_group_id]]).to_json
				when "post"
	...
	def get_user_id
		begin
			id = ActiveRecord::Base.connection.select_value("select dbo.idgenerator('renew_users', 'id', 'renew_web')")
			render :text => {"id" => "#{id}"}.to_json
		rescue => t
			puts "Error!!! #{t}"
			render :text => {"success" => false, "msg" => "#{t}"}.to_json
		end
	...
	def get_group_id
		begin
			id = ActiveRecord::Base.connection.select_value("select dbo.idgenerator('renew_user_group', 'id', 'renew_web')")
			render :text => {"id" => "#{id}"}.to_json
		rescue => t
			puts "Error!!! #{t}"
			render :text => {"success" => false, "msg" => "#{t}"}.to_json
